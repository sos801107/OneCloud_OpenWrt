...
#固件版本号添加个人标识和日期
[ -e package/lean/default-settings/files/zzz-default-settings ] && sed -i "s/DISTRIB_DESCRIPTION='.*OpenWrt '/DISTRIB_DESCRIPTION='OneCloud($(TZ=UTC-8 date +%Y.%m.%d)) '/g" package/lean/default-settings/files/zzz-default-settings
...
[ ! -e package/lean/default-settings/files/zzz-default-settings ] && echo "DISTRIB_DESCRIPTION='OneCloud ($(TZ=UTC-8 date +%Y.%m.%d)) '" >> package/base-files/files/etc/openwrt_release
